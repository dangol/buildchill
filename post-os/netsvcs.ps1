...
function createvars {
  ...
  # *********Edit this section***********
  # deprecated $ROLE = 'GAMESERVERP' #Determines Primary, GAMESERVERB, or Backup, GAMESERVERB configuration
  ...
  # **********End Editing here!************
'@
  set-content $INPATH\helpers\vars.ps1 $VARS
}
...
If (Test-Path $INPATH\helpers\vars.ps1) {
  . $INPATH\helpers\vars.ps1
} Else {
...
# Script Body starts
write-host "Computer name is: $env:computername, role determined by [P]rimary or [B]ackup in name"
...
if ($env:computername.EndsWith('P','CurrentCultureIgnoreCase')) {
  $dnsP=primaryDNS
  $dhcpP=primaryDHCP  
}
elseif ($env:computername.EndsWith('B','CurrentCultureIgnoreCase')) {
  $dnsB=secondaryDNS
  $dhcpB=secondaryDHCP
}
